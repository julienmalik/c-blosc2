...
void delta_decoder8(blosc2_sheader* sheader, uint8_t* filters_chunk, int32_t offset, int32_t nbytes, uint8_t* dest) {
  int i;
  int32_t rbytes = *(int32_t*)(filters_chunk + 4);
  int32_t mbytes;
  uint8_t* dref;
  ...
  if ((sheader != NULL) && (sheader->reserved != NULL)) {
    dref = sheader->reserved;
  }
  else {
    /* Uncompressed reference is not there.  Decompress it. */
    dref = malloc((size_t)rbytes);
    dctx = blosc2_create_dctx(&dparams);
    mbytes = blosc2_decompress_ctx(dctx, filters_chunk, (void*)dref, (size_t)rbytes);
    blosc2_free_ctx(dctx);
    if (mbytes < 0) {
      printf("Error in decompressing reference (delta_decoder8).  Please report this!\n");
      return;
    }
  }
  if (sheader != NULL) {
    /* Put the header in the cache */
    sheader->reserved = dref;
  }
  ...
  if (sheader == NULL) {
    /* We don't have any place to cache the reference, so free it. */
    free(dref);
  }
